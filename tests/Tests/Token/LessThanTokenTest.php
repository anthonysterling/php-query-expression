...
class LessThanTokenTest extends PHPUnit_Framework_TestCase
{
    public function test_that_key_is_set()
    {
        $key = 'key';

        $token = new QueryExpression\Token\LessThanToken($key, 1);
    ...
    public function test_that_type_is_correct()
    {
        ...
        $this->assertEquals($token->getType(), QueryExpression\Token\Token::LESSTHAN);
    }
    ...
    public function test_that_value_is_set()
    {
        ...
        $token = new QueryExpression\Token\LessThanToken('key', $value);
    ...
    {
        new QueryExpression\Token\LessThanToken('key', $value);
    }
    ...
    {
        $token = QueryExpression\Token\LessThanToken::fromString('key;less-than=5');

        $this->assertInstanceOf(QueryExpression\Token\LessThanToken::class, $token);
    }
}
