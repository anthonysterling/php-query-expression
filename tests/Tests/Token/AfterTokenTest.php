...
class AfterTokenTest extends PHPUnit_Framework_TestCase
{
    public function test_that_key_is_set()
    {
        $key = 'updated_at';

        $token = new QueryExpression\Token\AfterToken($key, '1980-01-29T08:30:45');
    ...
    {
        $token = new QueryExpression\Token\AfterToken('updated_at', '1980-01-29T08:30:45');

        $this->assertEquals($token->getType(), QueryExpression\Token\Token::AFTER);
    }
    ...
    public function test_that_value_is_set()
    {
        ...
        $token = new QueryExpression\Token\AfterToken('updated_at', $value);
    ...
    {
        new QueryExpression\Token\AfterToken('key', $value);
    }
    ...
    public function test_that_token_can_be_created_from_string()
    {
        foreach (['updated_at;after=1980-01-29T08:30:45', 'updated_at;after=1980-01-29 08:30:45'] as $string) {
            $token = QueryExpression\Token\AfterToken::fromString($string);
            $this->assertInstanceOf(QueryExpression\Token\AfterToken::class, $token);
        }
    }
}
