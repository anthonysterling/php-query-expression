...
class IsOrMoreThanTokenTest extends PHPUnit_Framework_TestCase
{
    public function test_that_key_is_set()
    {
        $key = 'key';

        $token = new QueryExpression\Token\IsOrMoreThanToken($key, 1);
    ...
    public function test_that_type_is_correct()
    {
        ...
        $this->assertEquals($token->getType(), QueryExpression\Token\Token::ISORMORETHAN);
    }
    ...
    public function test_that_value_is_set()
    {
        ...
        $token = new QueryExpression\Token\IsOrMoreThanToken('key', $value);
    ...
    {
        new QueryExpression\Token\IsOrMoreThanToken('key', $value);
    }
    ...
    {
        $token = QueryExpression\Token\IsOrMoreThanToken::fromString('key;is-or-more-than=5');

        $this->assertInstanceOf(QueryExpression\Token\IsOrMoreThanToken::class, $token);
    }
}
